-- Client script for No-NPC, refactored for performance and clarity.

-- Global state variables controlled by the server
local npcState = true
local vehicleState = true
local safeZones = {}
local protectedVehicles = {} -- table to hold netIds of protected vehicles
local currentServerHour = 12 -- Default to noon until first sync
local timeMultiplier = { ped = 1.0, vehicle = 1.0 }

-- Helper function to check if a value exists in a table
local function table_contains(tbl, element)
    for _, value in pairs(tbl) do
        if value == element then
            return true
        end
    end
    return false
end

-- Helper function to calculate distance between two vectors
local function GetDistance(vec1, vec2)
    return #(vec1 - vec2)
end

-- #############################################################################
-- ## CORE LOGIC FUNCTIONS
-- #############################################################################

-- Updates the time-based multipliers based on the current server hour
local function updateTimeBasedMultipliers()
    local activeProfileFound = false
    -- Default multipliers
    timeMultiplier.ped = 1.0
    timeMultiplier.vehicle = 1.0

    for _, profile in ipairs(Config.TimeProfiles) do
        local isOvernight = profile.startHour > profile.endHour
        local isActive = false
        if isOvernight then
            if currentServerHour >= profile.startHour or currentServerHour < profile.endHour then isActive = true end
        else
            if currentServerHour >= profile.startHour and currentServerHour < profile.endHour then isActive = true end
        end

        if isActive then
            timeMultiplier.ped = profile.pedMultiplier
            timeMultiplier.vehicle = profile.vehicleMultiplier
            activeProfileFound = true
            if Config.DebugMode then print(('[no-npc] Time profile "%s" is now active.'):format(profile.name)) end
            break
        end
    end
end


-- #############################################################################
-- ## EVENT HANDLERS (Server Communication)
-- #############################################################################

RegisterNetEvent('no_npc:updateStates')
AddEventHandler('no_npc:updateStates', function(newNpcState, newVehicleState)
    npcState = newNpcState
    vehicleState = newVehicleState
    if Config.DebugMode then print(('[no-npc] State updated: NPCs=%s, Vehicles=%s'):format(tostring(npcState), tostring(vehicleState))) end
end)

RegisterNetEvent('updateSafeZones')
AddEventHandler('updateSafeZones', function(newSafeZones)
    safeZones = newSafeZones
    if Config.DebugMode then print(('[no-npc] Received %d safe zones.'):format(#safeZones)) end
end)

RegisterNetEvent('no_npc:syncProtectedVehicles')
AddEventHandler('no_npc:syncProtectedVehicles', function(syncedProtectedVehicles)
    protectedVehicles = syncedProtectedVehicles
    if Config.DebugMode then print(('[no-npc] Received %d protected vehicles.'):format(#protectedVehicles)) end
end)

RegisterNetEvent('no_npc:syncTime')
AddEventHandler('no_npc:syncTime', function(hour)
    currentServerHour = hour
    if Config.DebugMode then print(('[no-npc] Server time synced. Current hour: %d'):format(hour)) end
    -- Immediately update multipliers when time is synced
    updateTimeBasedMultipliers()
end)


-- #############################################################################
-- ## ENTITY REMOVAL & SPAWNING FUNCTIONS
-- #############################################################################

local function isInSafeZone(coords)
    if not safeZones or #safeZones == 0 then return false end
    for _, zone in ipairs(safeZones) do
        if GetDistance(coords, zone.coords) <= zone.radius then return true end
    end
    return false
end

local function removeNPCs()
    if not npcState then return end
    if not Config.ExemptNPCHashes then
        Config.ExemptNPCHashes = {}
        for _, modelName in ipairs(Config.ExemptNPCs) do table.insert(Config.ExemptNPCHashes, GetHashKey(modelName)) end
    end
    local peds = GetGamePool('CPed')
    for _, ped in ipairs(peds) do
        if DoesEntityExist(ped) and not IsPedAPlayer(ped) then
            if Config.IgnoreMissionEntities and IsEntityAMissionEntity(ped) then goto continue end
            if not isInSafeZone(GetEntityCoords(ped)) then
                if not table_contains(Config.ExemptNPCHashes, GetEntityModel(ped)) then DeleteEntity(ped) end
            end
        end
        ::continue::
    end
end

local function removeVehicles()
    if not vehicleState then return end
    if not Config.AllowedVehicleHashes then
        Config.AllowedVehicleHashes = {}
        for _, modelName in ipairs(Config.AllowedVehicles) do table.insert(Config.AllowedVehicleHashes, GetHashKey(modelName)) end
    end
    local vehicles = GetGamePool('CVehicle')
    for _, vehicle in ipairs(vehicles) do
        if DoesEntityExist(vehicle) then
            if Config.IgnoreMissionEntities and IsEntityAMissionEntity(vehicle) then goto continue end
            if protectedVehicles[VehToNet(vehicle)] then goto continue end
            if not isInSafeZone(GetEntityCoords(vehicle)) then
                local driver = GetPedInVehicleSeat(vehicle, -1)
                if not (Config.IgnorePlayerVehicles and driver and IsPedAPlayer(driver)) then
                    if not table_contains(Config.AllowedVehicleHashes, GetEntityModel(vehicle)) then DeleteEntity(vehicle) end
                end
            end
        end
        ::continue::
    end
end

local function spawnWildlife()
    if not Config.SpawnWildlife or not npcState then return end
    local playerCoords = GetEntityCoords(PlayerPedId())
    if GetDistance(playerCoords, Config.WildlifeSpawnArea.center) <= Config.WildlifeSpawnArea.radius then
        local modelHash = GetHashKey(Config.WildlifeModels[math.random(#Config.WildlifeModels)])
        RequestModel(modelHash)
        local timeout = 1000
        while not HasModelLoaded(modelHash) and timeout > 0 do Citizen.Wait(100); timeout = timeout - 100 end
        if HasModelLoaded(modelHash) then
            local spawnCoords = playerCoords + vector3(math.random(-50, 50), math.random(-50, 50), 0)
            local success, z = GetGroundZFor_3dCoord(spawnCoords.x, spawnCoords.y, spawnCoords.z + 50.0, false)
            if success then
                local animal = CreatePed(28, modelHash, vector3(spawnCoords.x, spawnCoords.y, z), math.random(0, 360), true, false)
                TaskWanderStandard(animal, 10.0, 10)
                SetEntityAsNoLongerNeeded(animal)
            end
        end
        SetModelAsNoLongerNeeded(modelHash)
    end
end

-- #############################################################################
-- ## MAIN THREADS
-- #############################################################################

-- High-performance loop for frame-by-frame natives
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        local playerCoords = GetEntityCoords(PlayerPedId())
        local currentPedDensity = Config.DefaultDensity.ped
        local currentVehicleDensity = Config.DefaultDensity.vehicle
        for _, zone in ipairs(Config.DensityZones) do
            if GetDistance(playerCoords, zone.center) <= zone.radius then
                currentPedDensity = zone.pedDensity
                currentVehicleDensity = zone.vehicleDensity
                break
            end
        end
        if not npcState then currentPedDensity = 0.0 end
        if not vehicleState then currentVehicleDensity = 0.0 end
        currentPedDensity = currentPedDensity * timeMultiplier.ped
        currentVehicleDensity = currentVehicleDensity * timeMultiplier.vehicle
        SetPedDensityMultiplierThisFrame(currentPedDensity)
        SetScenarioPedDensityMultiplierThisFrame(currentPedDensity, currentPedDensity)
        SetVehicleDensityMultiplierThisFrame(currentVehicleDensity)
        SetRandomVehicleDensityMultiplierThisFrame(currentVehicleDensity)
        SetParkedVehicleDensityMultiplierThisFrame(currentVehicleDensity)
        SetGarbageTrucks(false)
        SetRandomBoats(false)
        SetCreateRandomCops(false)
    end
end)

-- Slower loop for cleanup
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(Config.CleanupInterval)
        removeNPCs()
        removeVehicles()
    end
end)

-- Loop for spawning wildlife
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(Config.WildlifeSpawnInterval)
        spawnWildlife()
    end
end)

-- Request initial data from server on startup
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    TriggerServerEvent('no_npc:requestInitialData')
end)
