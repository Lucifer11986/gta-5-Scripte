...
function setTabletVisible(visible)
    ...
    -- If we are opening the tablet, send the latest data lists
    if visible then
        SendNuiMessage(json.encode({
...
-- Register a command to toggle duty status
RegisterCommand('dienst', function()
    TriggerServerEvent('jobcore:toggleDuty')
    -- A proper chat message would be better here.
    print("Duty status toggled. The server will confirm your new status.")
end, false)
...
-- Listen for the server event that sends updated duty status
RegisterNetEvent('jobcore:updateDutyStatus')
AddEventHandler('jobcore:updateDutyStatus', function(players)
