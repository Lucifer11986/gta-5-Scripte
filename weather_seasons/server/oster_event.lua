...
-- Holt zufällige Ei-Positionen
function getRandomEggLocations(num)
    local randomLocations = {}

    -- Warte auf die Config
    while Config == nil or Config.EggLocations == nil do
        Wait(1000)
    end
...
function StartEasterEvent()
    ...
    -- Setze neue, zufällige Positionen
    if Config.EggLocations and #Config.EggLocations > 0 then
        ...
        print("[Oster-Event] Keine Osterei-Standorte in der Config gefunden.")
    end
end
...
-- Initiale Prüfung beim Serverstart
CreateThread(function()
    Wait(5000) -- Kurze Verzögerung
    if exports.weather_seasons:GetCurrentSeason() == "Frühling" then
        StartEasterEvent()
    end
end)

-- Eier spawnen für einen Spieler
