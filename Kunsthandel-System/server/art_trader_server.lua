...
-- Kunstwerke und Spieler-Kunstwerke an den Client senden
RegisterServerEvent('art_trade:requestArtworks')
AddEventHandler('art_trade:requestArtworks', function()
    local src = source
    GetArtworks(function(artworks)
        MySQL.Async.fetchAll('SELECT * FROM player_artworks WHERE identifier = @identifier', {
            ['@identifier'] = ESX.GetPlayerFromId(src).identifier or QBCore.Functions.GetPlayer(src).PlayerData.citizenid
        }, function(playerArtworks)
            TriggerClientEvent('art_trade:setArtworks', src, artworks, playerArtworks)
        end)
    end)
end)